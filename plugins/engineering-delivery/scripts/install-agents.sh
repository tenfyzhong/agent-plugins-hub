...
usage() {
    printf '%s\n' \
        "Usage: $0 [--platform codex|claude|pi|omp|all] [--force]" \
        "" \
        "Install Engineering Delivery agent templates into user configuration directories." \
        "Existing customized files are preserved unless --force is supplied."
}
...
install_platform() {
    ...
    for agent_name in "${agent_names[@]}"; do
        ...
        if [[ ! -e "$destination_file" ]]; then
            ...
            install -m 0644 "$source_file" "$destination_file"
            printf 'replaced customized: %s (backup: %s)\n' "$destination_file" "$backup_path"
        else
            printf 'preserved customized: %s\n' "$destination_file"
        fi
    done
}
